...
- Frontend/UI: React components live in `src/renderer/` (see `src/renderer/components/`), styled with Mielo UI to provide the Adwaita look & feel — see `MIGRATION_TO_MIELO_UI.md` for migration notes
...
- Frontend: the UI is implemented with React and styled using Mielo UI to achieve an Adwaita look & feel. See `src/renderer/` and `MIGRATION_TO_MIELO_UI.md` for guidance when modifying UI components and styles.
